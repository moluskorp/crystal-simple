...
ipcMain.handle(
  IPC.STORES.CHECK_EXISTS,
  async (): Promise<str.CheckStoreExistsResponse> => {
    try {
...
ipcMain.handle(
  IPC.STORES.CREATE,
  async (_, data: str.CreateStoreRequest): Promise<str.CreateStoreResponse> => {
    try {
...
ipcMain.handle(
  IPC.STORES.UPDATE,
  async (_, data: str.UpdateStoreRequest): Promise<str.UpdateStoreResponse> => {
    try {
